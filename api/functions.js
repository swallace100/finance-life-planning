  ...
  handler: async (req, context) => {
    try {
      ...
      return {
        status: 400,
        body: JSON.stringify({
          error: "Invalid file — upload a valid .xlsx workbook",
        }),
      };
    }
  },
...
app.http("save-row", {
  ...
  handler: async (req, context) => {
    ...
    if (!sheetName || !row) {
      return {
        status: 400,
        body: JSON.stringify({ error: "sheetName and row are required" }),
      };
    }

    try {
      const workbook = await loadWorkbook();
      const { sheet, columnMap, colIndexToHeader, maxCol } = ensureSheet(
        workbook,
        sheetName,
      );
      ...
      if (isNew) {
        if (columnMap["ID"]) {
          ...
          sheet.eachRow((row, i) => {
            if (i === 1) return;
            const id = row.getCell(columnMap["ID"]).value;
        ...
        for (let col = 1; col <= maxCol; col++) {
          const header = colIndexToHeader[col];
          values.push(
            header && cleanRow[header] !== undefined
              ? prepareValue(cleanRow[header])
              : null,
          );
        }
        sheet.addRow(values);
        ...
        if (!_rowIndex) {
          return {
            status: 400,
            body: JSON.stringify({ error: "No _rowIndex provided for edit" }),
          };
        }
      ...
      return {
        status: 500,
        body: JSON.stringify({ error: "Failed to save row" }),
      };
    }
  },
});
...
app.http("delete-row", {
  ...
  handler: async (req, context) => {
    ...
    if (!sheetName || !rowIndex) {
      return {
        status: 400,
        body: JSON.stringify({ error: "sheetName and rowIndex are required" }),
      };
    }
    ...
    try {
      ...
      return {
        status: 500,
        body: JSON.stringify({ error: "Failed to delete row" }),
      };
    }
  },
});
