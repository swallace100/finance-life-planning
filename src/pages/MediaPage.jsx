...
const STATUS_STYLE = {
  ...
  "100% Completed": "text-amber-400 bg-amber-400/10",
...
export default function MediaPage({ data, onSave, onDelete }) {
  ...
  const filteredBooks = search.books
    ? books.filter(
        (b) =>
          q(b.Name).includes(q(search.books)) ||
          q(b.Author).includes(q(search.books)) ||
          q(b.Genre).includes(q(search.books)),
      )
    : books;
  const filteredFilms = search.films
    ? films.filter(
        (f) =>
          q(f.Name).includes(q(search.films)) ||
          q(f.ReleaseYear).includes(q(search.films)),
      )
    : films;
  const filteredGames = search.games
    ? games.filter(
        (g) =>
          q(g.Name).includes(q(search.games)) ||
          q(g.Platform).includes(q(search.games)) ||
          q(g.Genre).includes(q(search.games)),
      )
    : games;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-slate-800/50 p-1 rounded-xl border border-slate-700 w-fit">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            <span
              className={`text-xs tabular-nums px-1.5 py-0.5 rounded-full ${activeTab === t.id ? "bg-slate-600 text-slate-300" : "bg-slate-700/50 text-slate-500"}`}
            >
              {t.count}
            </span>
          </button>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-4 mb-5">
            <div className="flex-1">
              <SearchBar
                value={search.books}
                onChange={(v) => setTabSearch("books", v)}
          ...
          {(search.books || bookPagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.books
                ? `${filteredBooks.length} of ${books.length} entries`
          ...
          {booksSorted.length === 0 ? (
            <p className="text-slate-500 text-sm">
              {search.books ? "No matches." : "No books logged yet."}
            </p>
          ) : (
            <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700 text-xs font-semibold uppercase tracking-wide">
                      <SortableHeader
                        col="Name"
                        label="Title"
                        sortKey={bookSort.sortKey}
                        sortDir={bookSort.sortDir}
                        onSort={bookSort.handleSort}
                        className="pb-2 pr-4"
                      />
                      <SortableHeader
                        col="Author"
                  ...
                  <tbody>
                    {bookPagination.pageItems.map((b) => (
                      <tr
                        key={b.ID ?? b._rowIndex}
                        className={rowHover}
                        onClick={() => bookModal.openEdit(b)}
                      >
                        ...
                        <td className="py-2.5 pr-4">
                          {b.Format ? (
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full ${FORMAT_STYLE[b.Format] ?? "text-slate-400 bg-slate-700"}`}
                            >
                              {b.Format}
                            </span>
                          ) : (
                        ...
                        <td className="py-2.5 pr-4 text-slate-400 text-right text-xs">
                          {fmtDate(b.ReadDate)}
                        </td>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-4 mb-5">
            <div className="flex-1">
              <SearchBar
                value={search.films}
                onChange={(v) => setTabSearch("films", v)}
          ...
          {(search.films || filmPagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.films
                ? `${filteredFilms.length} of ${films.length} entries`
                : `${films.length} entries`}
            </p>
          )}

          {filmsSorted.length === 0 ? (
            <p className="text-slate-500 text-sm">
              {search.films ? "No matches." : "No films logged yet."}
            </p>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700 text-xs font-semibold uppercase tracking-wide">
                      <SortableHeader
                        col="Name"
                        label="Title"
                        sortKey={filmSort.sortKey}
                        sortDir={filmSort.sortDir}
                        onSort={filmSort.handleSort}
                        className="pb-2 pr-4"
                      />
                      <SortableHeader
                        col="ReleaseYear"
                  ...
                  <tbody>
                    {filmPagination.pageItems.map((f) => (
                      <tr
                        key={f.ID ?? f._rowIndex}
                        className={rowHover}
                        onClick={() => filmModal.openEdit(f)}
                      >
                        <td className="py-2.5 pr-4 text-slate-200 font-medium">
                          {f.Name}
                        </td>
                        <td className="py-2.5 pr-4 text-slate-500 text-right tabular-nums text-xs">
                          {f.ReleaseYear || "—"}
                        </td>
                        <td className="py-2.5 pr-4 text-slate-400 text-right text-xs">
                          {fmtDate(f.WatchDate)}
        ...
        <div className="card p-6">
          <div className="flex items-center gap-4 mb-5">
            <div className="flex-1">
              <SearchBar
                value={search.games}
                onChange={(v) => setTabSearch("games", v)}
          ...
          {(search.games || gamePagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.games
                ? `${filteredGames.length} of ${games.length} entries`
          ...
          {gamesSorted.length === 0 ? (
            <p className="text-slate-500 text-sm">
              {search.games ? "No matches." : "No games logged yet."}
            </p>
          ) : (
            <>
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {gamePagination.pageItems.map((g) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 pr-4">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${STATUS_STYLE[g.Status] ?? "text-slate-400 bg-slate-700"}`}
                          >
                            {g.Status}
                          </span>
                        </td>
                        <td className="py-2.5 pr-4 text-slate-400 text-right text-xs">
                          {fmtDate(g.CompletionDate)}
                        </td>
                        <td className="py-2.5 text-right">
      ...
      <Modal
        ...
        title={bookModal.isEditing ? "Edit Book" : "Add Book"}
      >
        <EntityForm
          ...
          onDelete={
            bookModal.isEditing
              ? () => handleBookDelete(bookModal.editRow)
              : undefined
          }
        />
      </Modal>
      <Modal
        open={filmModal.open}
        onClose={filmModal.close}
        title={filmModal.isEditing ? "Edit Film" : "Add Film"}
      >
        <EntityForm
          ...
          onDelete={
            filmModal.isEditing
              ? () => handleFilmDelete(filmModal.editRow)
              : undefined
          }
        />
      </Modal>
      <Modal
        ...
        title={gameModal.isEditing ? "Edit Game" : "Add Game"}
      >
        <EntityForm
          ...
          onDelete={
            gameModal.isEditing
              ? () => handleGameDelete(gameModal.editRow)
              : undefined
          }
        />
      </Modal>
