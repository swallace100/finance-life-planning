...
function SearchBar({ value, onChange, placeholder }) {
  return (
    <div className="relative">
      ...
      />
      {value && (
        <button onClick={() => onChange("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300">✕</button>
      )}
    </div>
  );
}
...
export default function MediaPage({ data, onSave, onDelete }) {
  ...
  const bookPagination = usePagination(booksSorted, PAGE_SIZE);
  const filmPagination = usePagination(filmsSorted, PAGE_SIZE);
  const gamePagination = usePagination(gamesSorted, PAGE_SIZE);

  async function handleBookSubmit(row) { await onSave("ReadingLog", row, row._rowIndex == null); bookModal.close(); }
  async function handleBookDelete(row) { await onDelete("ReadingLog", row._rowIndex); bookModal.close(); }
  async function handleFilmSubmit(row) { await onSave("FilmLog",     row, row._rowIndex == null); filmModal.close(); }
  async function handleFilmDelete(row) { await onDelete("FilmLog",   row._rowIndex); filmModal.close(); }
  async function handleGameSubmit(row) { await onSave("GamingLog",   row, row._rowIndex == null); gameModal.close(); }
  async function handleGameDelete(row) { await onDelete("GamingLog", row._rowIndex); gameModal.close(); }
    ...
    <div className="space-y-6">
        ...
        {TABS.map(t => (
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${
              activeTab === t.id ? "bg-slate-700 text-slate-100 shadow" : "text-slate-400 hover:text-slate-200"
            }`}
          >
            {t.label}
      ...
      {activeTab === "books" && (
        <div className="card p-6">
          ...
          {(search.books || bookPagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.books ? `${filteredBooks.length} of ${books.length} entries` : `${books.length} entries`}
            </p>
          )}
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {bookPagination.pageItems.map(b => (
                      <tr key={b.ID ?? b._rowIndex} className={rowHover} onClick={() => bookModal.openEdit(b)}>
                        ...
                        <td className="py-2.5 pr-4">
                          {b.Format ? <span className={`text-xs px-2 py-0.5 rounded-full ${FORMAT_STYLE[b.Format] ?? "text-slate-400 bg-slate-700"}`}>{b.Format}</span> : <span className="text-slate-600">—</span>}
                        </td>
      ...
      {activeTab === "films" && (
        <div className="card p-6">
          ...
          {(search.films || filmPagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.films ? `${filteredFilms.length} of ${films.length} entries` : `${films.length} entries`}
            </p>
          )}
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filmPagination.pageItems.map(f => (
      ...
      {activeTab === "games" && (
        <div className="card p-6">
          ...
          {(search.games || gamePagination.totalPages > 1) && (
            <p className="text-slate-500 text-xs mb-3">
              {search.games ? `${filteredGames.length} of ${games.length} entries` : `${games.length} entries`}
            </p>
          )}
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {gamePagination.pageItems.map(g => (
                      <tr key={g.ID ?? g._rowIndex} className={rowHover} onClick={() => gameModal.openEdit(g)}>
                        ...
                        <td className="py-2.5 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_STYLE[g.Status] ?? "text-slate-400 bg-slate-700"}`}>{g.Status}</span>
                        </td>
      ...
      <Modal open={bookModal.open} onClose={bookModal.close} title={bookModal.isEditing ? "Edit Book" : "Add Book"}>
        <EntityForm schema={SCHEMAS.ReadingLog} initialValues={bookModal.editRow} data={data} isEditing={bookModal.isEditing} onSubmit={handleBookSubmit} onCancel={bookModal.close} onDelete={bookModal.isEditing ? () => handleBookDelete(bookModal.editRow) : undefined} />
      </Modal>
      <Modal open={filmModal.open} onClose={filmModal.close} title={filmModal.isEditing ? "Edit Film" : "Add Film"}>
        <EntityForm schema={SCHEMAS.FilmLog} initialValues={filmModal.editRow} data={data} isEditing={filmModal.isEditing} onSubmit={handleFilmSubmit} onCancel={filmModal.close} onDelete={filmModal.isEditing ? () => handleFilmDelete(filmModal.editRow) : undefined} />
      </Modal>
      <Modal open={gameModal.open} onClose={gameModal.close} title={gameModal.isEditing ? "Edit Game" : "Add Game"}>
        <EntityForm schema={SCHEMAS.GamingLog} initialValues={gameModal.editRow} data={data} isEditing={gameModal.isEditing} onSubmit={handleGameSubmit} onCancel={gameModal.close} onDelete={gameModal.isEditing ? () => handleGameDelete(gameModal.editRow) : undefined} />
      </Modal>
    </div>
  );
}
