...
const CLASS_ORDER = ['Cash', 'Bonds/CD', 'Stock', 'Other']
...
function computeTree(data) {
  ...
  cds.filter(cd => cd.Active !== false && cd.Active !== 0 && cd.Active !== 'No').forEach(cd => {
    add('Bonds/CD', 'Certificate Deposit', Number(cd.FaceValue) || 0)
  })

  // Investment/Retirement accounts with FundAllocation data
  ...
  fundAllocs.forEach(alloc => {
    const val = (holdingValues[alloc.HoldingID] || 0) * (Number(alloc.Percentage) / 100)
    const cls = subToClass[alloc.AssetClass] || 'Stock'
    add(cls, alloc.AssetClass, val)
  })

  // Investment/Retirement with no holdings data → Stock > Subtype/Name (unclassified)
  assets.filter(a => ['Investment', 'Retirement'].includes(a.Type) && !accountsWithHoldings.has(a.ID)).forEach(a => {
  ...
  tangibles.filter(a => a.StillHave !== false && a.StillHave !== 0).forEach(a => {
    add('Other', a.Category || 'Other', Number(a.CurrentValue) || 0)
  })

  let total = 0
  Object.values(tree).forEach(subs => Object.values(subs).forEach(v => { total += v }))
...
function Gap({ actual, goal }) {
  ...
  const cls = Math.abs(diff) <= 2 ? 'text-emerald-400' : Math.abs(diff) <= 7 ? 'text-amber-400' : 'text-red-400'
  return <span className={`tabular-nums ${cls}`}>{diff >= 0 ? '+' : ''}{diff.toFixed(1)}</span>
}

export default function AllocationPage({ data, onSave }) {
  ...
  const classes = Object.keys(tree).sort((a, b) => {
    const ai = CLASS_ORDER.indexOf(a); const bi = CLASS_ORDER.indexOf(b)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {total > 0 && (
            <p className="text-slate-500 text-sm mt-0.5">{fmtCurrency.format(total)} tracked across all sources</p>
          )}
        </div>
      </div>
      ...
      {hasFundAllocationGap && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl px-5 py-3">
          <p className="text-amber-400 text-sm font-medium">FundAllocation data needed for full stock breakdown</p>
          <p className="text-amber-400/70 text-xs mt-0.5">
            You have retirement holdings but no fund allocations. Add entries to FundAllocation on the Retirement page to break stocks down by asset class.
          </p>
        </div>
      )}
              ...
              return (
                <Fragment key={cls}>
                  <tr className="border-b border-slate-600 bg-slate-700/20">
                    <td className="px-6 py-3 font-semibold text-slate-100">{cls}</td>
                    <td className="px-4 py-3 text-right font-semibold tabular-nums">
                      <span className={gapCls(classPct, classGoal)}>{fmtPct(classPct)}</span>
                    </td>
                  ...
                  {sortedSubs.map(([sub, val]) => {
                    const subPct  = classTotal > 0 ? (val / classTotal) * 100 : 0
                    const subGoal = subGoals[`${cls}::${sub}`]
                    return (
                      <tr key={sub} className="border-b border-slate-700/30 last:border-0 hover:bg-slate-700/20 transition-colors">
                        <td className="pl-10 pr-6 py-2.5 text-slate-400">{sub}</td>
                        <td className="px-4 py-2.5 text-right text-slate-700">—</td>
                        <td className="px-4 py-2.5 text-right tabular-nums text-slate-300">{fmtPct(subPct)}</td>
                        <td className="px-4 py-2.5 text-right text-slate-700">—</td>
            ...
            {classes.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-10 text-center text-slate-500">
                  No allocation data found. Connect your Excel file to see your portfolio breakdown.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700">
        ...
        >
          <span className="text-slate-300 font-medium text-sm">
            Goals Configuration
            {goals.length > 0 && <span className="ml-2 text-slate-500 font-normal">{goals.length} entries</span>}
          </span>
          <div className="flex items-center gap-3">
            <span
              role="button"
              onClick={e => { e.stopPropagation(); goalsModal.openAdd() }}
          ...
          <div className="border-t border-slate-700 px-6 pb-5">
            {goals.length === 0 ? (
              <p className="text-slate-500 text-sm py-4">
                No goals configured. Add entries to see how your actual allocation compares to targets.
                Leave Subclass blank for a class-level goal (e.g. "Stock = 65%"), or fill it in for
                a within-class target (e.g. "US Stocks = 60% of Stock").
              </p>
            ) : (
              <table className="w-full text-sm mt-4">
                ...
                <tbody>
                  {goals.map(g => (
                    ...
                    >
                      <td className="py-2.5 pr-4 text-slate-200">{g.AssetClass}</td>
                      <td className="py-2.5 pr-4 text-slate-400">
                        {g.Subclass || <span className="text-slate-600 italic text-xs">class level</span>}
                      </td>
                      <td className="py-2.5 text-right text-slate-300 tabular-nums">{g.GoalPct}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Modal open={goalsModal.open} onClose={goalsModal.close} title={goalsModal.isEditing ? 'Edit Goal' : 'Add Goal'}>
        <EntityForm
