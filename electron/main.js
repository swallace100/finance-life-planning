...
app.whenReady().then(() => {
  ...
  ipcMain.handle("save-row", async (_, { sheetName, row, isNew }) => {
    ...
    Object.entries(columnMap).forEach(([h, c]) => {
      colIndexToHeader[c] = h;
    });
    ...
    let nextCol = Object.values(columnMap).length
      ? Math.max(...Object.values(columnMap)) + 1
      : 1;
    ...
    let maxCol = Object.values(columnMap).length
      ? Math.max(...Object.values(columnMap))
      : 0;
    ...
    if (isNew) {
      ...
      for (let col = 1; col <= maxCol; col++) {
        ...
        const value =
          header && cleanRow[header] !== undefined ? cleanRow[header] : null;
        values.push(prepareValue(value));
      }
      sheet.addRow(values);
    } else {
      if (!_rowIndex)
        throw new Error("No _rowIndex provided for edit operation");
